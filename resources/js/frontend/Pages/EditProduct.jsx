...
export function EditProduct() {
  ...
  const LazyLoadingMarkup = productsLoading ? (
    <Listbox.Loading
      accessibilityLabel={`${queryValue ? "Filtering" : "Loading"} Products`}
    />
  ...
  const handlePrice = useCallback((value) => setPrice(value), []);

  const handleSku = useCallback((value) => setSku(value), []);
  const handleBarcode = useCallback((value) => setBarcode(value), []);

  const handleQuantity = useCallback((value) => setQuantity(value), []);
